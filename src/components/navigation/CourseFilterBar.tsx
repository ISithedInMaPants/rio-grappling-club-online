...
import { CourseLevel, GiFormat, PositionCategory } from '@/types';
...
export const CourseFilterBar: React.FC<CourseFilterBarProps> = ({
  ...
  const formats: { id: GiFormat | 'all'; label: string }[] = [
    ...
    { id: 'gi', label: 'Gi' },
    { id: 'nogi', label: 'No-Gi' },
    { id: 'both', label: 'Gi & No-Gi' },
  ];

  const levels: { id: CourseLevel | 'all'; label: string; colorDot?: string }[] = [
    { id: 'all', label: 'All Levels' },
    { id: 'beginner', label: 'Beginner', colorDot: 'bg-emerald-400' },
    { id: 'intermediate', label: 'Intermediate', colorDot: 'bg-blue-400' },
    { id: 'advanced', label: 'Advanced', colorDot: 'bg-amber-400' },
  ];
  ...
  return (
    <div className={`flex flex-col gap-4 bg-[#161619] border border-[#2b2b32] rounded-2xl p-4 md:p-5 ${className}`}>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        <div className="relative w-full md:max-w-md">
          ...
          <input
            ...
            placeholder="Search techniques, positions, gi, nogi, beginner..."
      ...
      {/* Second row: Skill Level filter pills */}
      <div className="flex flex-wrap items-center gap-1.5 pt-3 border-t border-[#2b2b32]">
        ...
        {levels.map((lvl) => {
          ...
          return (
            ...
            >
              {lvl.colorDot && (
                <span className={`w-2 h-2 rounded-full ${lvl.colorDot}`} />
              )}
              <span>{lvl.label}</span>
            </button>
          );
